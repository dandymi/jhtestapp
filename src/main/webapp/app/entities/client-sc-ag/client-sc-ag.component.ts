...
@Component({
  selector: 'jhi-client-sc-ag',
  templateUrl: './client-sc-ag.component.html'
})
export class ClientScAgComponent implements OnInit, OnDestroy {
  clients: IClientScAg[];
  currentAccount: any;
  ...
  loadAll() {
    this.clientService
      ...
      .subscribe(
        (res: HttpResponse<IClientScAg[]>) => this.paginateClients(res.body, res.headers),
  ...
  trackId(index: number, item: IClientScAg) {
    return item.id;
  }
  ...
  protected paginateClients(data: IClientScAg[], headers: HttpHeaders) {
